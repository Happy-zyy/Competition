    ...
    with tf.Session(config=config) as sess:
        ...
        with tf.variable_scope('training_ops') as vs:
            ...
            with tf.variable_scope('Optimizer1'):
                ...
                train_op1 = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(model.loss, var_list=tvars1)

            with tf.variable_scope('Optimizer2'):
                tvars2 = [tvar for tvar in tvars1 if 'embedding' not in tvar.name]
                train_op2 = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(model.loss, var_list=tvars2)
